...
if dmesg | grep "7z010" > /dev/null; then
        ...
        if dmesg | grep "7z020" > /dev/null; then
                ...
                printf "\nWriting 7z007.bit to QSPI Flash ${BIT_PART}\n"
                printf "0: %.8x" `stat -c %s $path/7z007.bit` | sed -e 's/0\: \(..\)\(..\)\(..\)\(..\)/0\: \4\3\2\1/' | xxd -r -g0 > /tmp/bitstream
                cat $path/7z007.bit >> /tmp/bitstream
                check_cmd "flashcp -v /tmp/bitstream ${BIT_PART}"
        fi
fi
